...
func UpdateInflightRequestMetrics(ctx context.Context, phase string, nonmutating, mutating int) {
	for _, kc := range []struct {
		kind  string
		count int
	}{{ReadOnlyKind, nonmutating}, {MutatingKind, mutating}} {
		if phase == ExecutingPhase {
			currentInflightRequests.WithContext(ctx).WithLabelValues(kc.kind).Set(float64(kc.count))
		} else {
			currentInqueueRequests.WithContext(ctx).WithLabelValues(kc.kind).Set(float64(kc.count))
		}
	}
}
